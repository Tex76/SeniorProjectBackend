...
// Used For data related to places
...
// Generate Trip Logic and API Call

app.post('/api/generate-itinerary', async (req, res) => {
  const { selectedRegion, groupSize, budget, days, favouriteActivities } = req.body;
  ...
  try {
    const response = await axios.post('https://api.openai.com/v1/chat/completions', {
      ...
      messages: [{
        ...
        role: 'user',
        content: `Generate an itinerary and a description for a trip within Bahrain to ${selectedRegion} 
        for a group of ${groupSize} 
        people with a budget of ${budget} 
        for ${days} days. 
        The favourite activities are ${favouriteActivities.join(', ')}.`,
      }],
      max_tokens: 200,
      temperature: 0.5
    }, {
    ...
    if (error.response.status === 429) {
      ...
      res.status(429).send(error.response.data.error.message);
